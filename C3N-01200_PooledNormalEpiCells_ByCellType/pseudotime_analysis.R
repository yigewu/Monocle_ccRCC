...
setwd(dir_base)
## set input paths
path_srat <- "./Resources/Analysis_Results/integration/30_aliquot_integration/docker_run_integration/20200212.v3/30_aliquot_integration.20200212.v3.RDS"
...
if (file.exists(obj_output_path) & opt$force.reprocess==FALSE){
  ...
  combined_subset <- subset(combined,cells=combined@meta.data %>% rownames_to_column %>% filter((Case == id_case & Cell_group.shorter=="Nephron_Epithelium") | Cell_type.shorter == "Normal epithelial cells") %>% .$rowname) 
