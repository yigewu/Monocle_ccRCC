...
## set number of tumor cells after down-sampling
number_tumorcells <- 392
...
if (file.exists(obj_output_path) & opt$force.reprocess==FALSE){
  ...
  ## add downsample idx
  idx_tumorcells_keep <- sample(x = which(barcode_to_celltype$Cell_type.shorter == "Tumor cells"), size = number_tumorcells)
  idx_nontumorcells_keep <- which(barcode_to_celltype$Cell_type.shorter != "Tumor cells" & barcode_to_celltype$Cell_group.shorter=="Nephron_Epithelium")
  idx_keep <- c(idx_tumorcells_keep, idx_nontumorcells_keep)
  barcode_to_celltype$Keep <- F
  barcode_to_celltype$Keep[idx_keep] <- T
