...
pdf(paste0(out_path,"cell_distribution_along_trajectory.pdf"),width=12,height=10)
        plot_cell_trajectory(combined_subset_pseudotime, color_by = "Aliquot.snRNA.WU",cell_size=0.3,cell_link_size=0.3,state_number_size=1)+facet_wrap(~State, nrow = 3)+theme(aspect.ratio=1)+guides(colour = guide_legend(override.aes = list(size=5)))
        plot_cell_trajectory(combined_subset_pseudotime, color_by = "Cell_type.detailed",cell_size=0.3)+facet_wrap(~State, nrow = 1)+theme(aspect.ratio=1)+guides(colour = guide_legend(override.aes = list(size=5)))
        plot_cell_trajectory(combined_subset_pseudotime, color_by = "State",cell_size=0.3)+theme(aspect.ratio=1)+guides(colour = guide_legend(override.aes = list(size=5)))
dev.off()
